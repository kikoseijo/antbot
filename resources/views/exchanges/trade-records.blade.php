...
<div class="overflow-x-auto relative sm:rounded-lg">
    <table class="w-full text-sm text-left text-gray-500 dark:text-gray-400">
        <thead class="text-xs text-gray-700 uppercase bg-gray-50 dark:bg-gray-700 dark:text-gray-400">
            <tr>
                <th scope="col" class="py-3 px-4 text-center w-10">Lev.</th>
                <th scope="col" class="py-3 px-4 text-center w-auto">Symbol</th>
                <th scope="col" class="py-3 px-4 text-center w-30 text-right">Date</th>
        ...
        <tbody>
          ...
          @endphp
            @forelse($records as $record)
              @php
                    if ($record->exchange->exchange == \App\Enums\ExchangesEnum::BYBIT) {

                        $side_text = $record->side == 'Buy' ? 'Short' : 'Long';
                        $side_color = $record->side == 'Buy' ? 'red' : 'green';
                    } else {
                        $side_text = $record->side == 'Buy' ? 'Long' : 'Short';
